...
const PATCHED_OPEN_TARGET_CONTEXT =
  `${OPEN_TARGET_LABEL_HELPER}function e({idPrefix:e,messages:t,onOpenInTarget:n,primaryTarget:r,visibleTargets:i}){return r==null?[]:[{id:\`${"${e}"}-primary\`,message:t.openInTarget,messageValues:{target:codexOpenTargetLabel(r)},icon:r.icon,onSelect:()=>n(r.target,r.appPath)},{id:\`${"${e}"}-targets\`,message:t.openIn,submenu:i.map(r=>({id:\`${"${e}"}-target-${"${r.id}"}\`,message:t.openInTargetSubmenu,messageValues:{target:codexOpenTargetLabel(r)},icon:r.icon,onSelect:()=>n(r.target,r.appPath)}))}]}`;
...
export function patchContextMenuMessageValueLabelsSource(source) {
  if (source.includes(CONTEXT_MENU_VALUES_HELPER)) {
    return source;
  }

  if (!source.includes(CONTEXT_MENU_PATTERN)) {
    throw new Error("Unable to patch context menu message values");
  }

  return source.replace(CONTEXT_MENU_PATTERN, PATCHED_CONTEXT_MENU);
}
...
export function patchOpenTargetContextMenuLabelsSource(source) {
  ...
  return source.replace(OPEN_TARGET_CONTEXT_PATTERN, PATCHED_OPEN_TARGET_CONTEXT);
}
...
export function patchWorkspaceFileContextMenuLabelsSource(source) {
  ...
  return (
    source.replace(WORKSPACE_FILE_CONTEXT_PATTERN, PATCHED_WORKSPACE_FILE_CONTEXT) +
...
export function patchWebviewOpenTargetLabelsAssets(assetsDir) {
  ...
  for (const { pattern, patch } of patchers) {
    const assetName = fs.readdirSync(assetsDir).find((name) => pattern.test(name));
    if (assetName == null) {
...
if (invokedPath === import.meta.url) {
  ...
  console.log(`Patched webview open target labels in ${patchedFiles.length} file(s)`);
}
